...
import { fetchCachedArrayBuffer, fetchCachedJson } from "./supertonic-model-cache.js";
...
const EXPRESSION_TAG_NAMES =
    'laugh|breath|sigh|surprise|scream|throatclear|sad|angry|cough|yawn';
const EXPRESSION_TAG_ONLY_RE = new RegExp(`^(?:<(?:${EXPRESSION_TAG_NAMES})>\\s*)+$`, 'i');
...
export class UnicodeProcessor {
    ...
    preprocessText(text, lang) {
        ...
        // Skip tag-only fragments — otherwise "<sigh>" becomes "<sigh>." and gets vocalized.
        if (!EXPRESSION_TAG_ONLY_RE.test(text) && !/[.!?;:,'\"')\]}…。」』】〉》›»]$/.test(text)) {
            text += '.';
        }
...
export async function loadVoiceStyle(voiceStylePaths, verbose = false) {
    ...
    const firstStyle = await fetchCachedJson(voiceStylePaths[0]);
    ...
    // Fill in the data
    for (let i = 0; i < bsz; i++) {
        const voiceStyle = await fetchCachedJson(voiceStylePaths[i]);
...
export async function loadCfgs(onnxDir) {
    return fetchCachedJson(`${onnxDir}/tts.json`);
}
...
export async function loadTextProcessor(onnxDir) {
    const indexer = await fetchCachedJson(`${onnxDir}/unicode_indexer.json`);
...
export async function loadOnnx(onnxPath, options) {
    const { buffer } = await fetchCachedArrayBuffer(onnxPath);
...
export async function loadTextToSpeech(onnxDir, sessionOptions = {}, progressCallback = null) {
    ...
    const totalSteps = modelPaths.length + 1;
    let completed = 0;

    const report = (name, phase) => {
        if (!progressCallback) return;
        progressCallback(name, completed, totalSteps, phase);
    };
    ...
    for (let i = 0; i < modelPaths.length; i++) {
        ...
        report(name, 'start');
        const session = await loadOnnx(path, sessionOptions);
